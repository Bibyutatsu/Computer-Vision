...
void eigenFaceTrainer(){
	...
	model->save("G:/yaml/eigenface.yml");
...
void fisherFaceTrainer(){
	...
	model->save("G:/yaml/fisherface.yml");
...
void LBPHFaceTrainer(){
	...
	model->save("G:/yaml/LBPHface.yml");
...
int  FaceRecognition(){
	...
	Ptr<FaceRecognizer>  model = createFisherFaceRecognizer();
	model->load("G:/yaml/fisherface.yml");

	Mat testSample = imread("G:/db/s41/5.pgm", 0);
